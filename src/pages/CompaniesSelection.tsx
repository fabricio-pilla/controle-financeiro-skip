...
import { Building2, Plus, ArrowRight, Users, LogOut, Sparkles } from 'lucide-react'

export default function CompaniesSelection() {
  const { user, logout } = useAuth()
  const { userCompanies, selectCompany } = useCompany()
      ...
      <main className="max-w-5xl w-full mx-auto my-auto py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {userCompanies.map((comp) => {
            ...
            return (
              <div
                ...
                className={`group relative bg-white border border-slate-200 hover:border-indigo-400 p-6 rounded-2xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[190px] ${
              ...
              >
                <div>
                  ...
                  {comp.cnpj && (
                    <p className="text-xs text-slate-400 font-mono mt-0.5">CNPJ: {comp.cnpj}</p>
                  )}
                </div>
          ...
          <button
            ...
            className="group min-h-[190px] border-2 border-dashed border-indigo-200 hover:border-indigo-500 bg-indigo-50/40 hover:bg-indigo-50/80 rounded-2xl p-6 flex flex-col items-center justify-center text-center transition-all duration-200 hover:shadow-md hover:-translate-y-1"
          >
